...
export default function NoticeReportPage() {
  ...
  const [filters, setFilters] = useState({ search: '', notice_type: '', priority: '', status: '', visibility: '', audience_type: '', committee: '', pinned: '', start_date: '', end_date: '', page: 1, per_page: 20, renderControls: (update) => (<><input className="ndm-input" value={filters.notice_type} onChange={(e) => update('notice_type', e.target.value)} placeholder="Notice type" /><select className="ndm-input" value={filters.priority} onChange={(e) => update('priority', e.target.value)}><option value="">All Priority</option><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select><select className="ndm-input" value={filters.visibility} onChange={(e) => update('visibility', e.target.value)}><option value="">All Visibility</option><option value="public">Public</option><option value="members_only">Members Only</option><option value="committee_specific">Committee Specific</option><option value="private">Private</option></select></>) });
