...
export default function PositionFormPage() {
  ...
  const prefill = useMemo(() => {
    ...
    if (!committeeTypeId && !committeeName) {
      return null;
    }

    return {
      committee_type_ids: committeeTypeId ? [committeeTypeId] : [],
      source_committee_name: committeeName || '',
      source_committee_type_name: committeeTypeName || '',
    ...
    <AdminContentWrapper>
      <PageContainer>
        ...
        <PageSection>
          ...
          {!loading || !isEdit ? <PositionForm initialValues={isEdit ? data : prefill} busy={Boolean(busyAction)} onCancel={() => navigate(isEdit ? `/admin/positions/${id}` : '/admin/positions')} onSubmit={(payload) => run(isEdit ? 'update' : 'create', id, payload)} /> : null}
        </PageSection>
      </PageContainer>
    </AdminContentWrapper>
  );
